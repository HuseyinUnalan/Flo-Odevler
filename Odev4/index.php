      ...
      <tbody>
        ...
        foreach ($sorgu as $satir) {
          ...
          if (strlen($satir["tckimlik"]) == 11) {
            $tekhanelertoplamı->tekhanelertoplamı = (substr($satir["tckimlik"], 0, 1) + substr($satir["tckimlik"], 2, 1) + substr($satir["tckimlik"], 4, 1) +
              substr($satir["tckimlik"], 6, 1) + substr($satir["tckimlik"], 8, 1));
          }
          if (strlen($satir["tckimlik"]) == 11) {
            $cifthanelerintoplamı->cifthanelerintoplamı = (substr($satir["tckimlik"], 1, 1) + substr($satir["tckimlik"], 3, 1) + substr($satir["tckimlik"], 5, 1) +
              substr($satir["tckimlik"], 7, 1));
          }

          if (strlen($satir["tckimlik"]) == 11) {
            $hanelerintoplam->hanelerintoplamı =  (substr($satir["tckimlik"], 0, 1) + substr($satir["tckimlik"], 1, 1) + substr($satir["tckimlik"], 2, 1) +
          ...
          $tckimlik->tckimlik = $satir["tckimlik"];

          $ilkhane->ilkhane = substr($satir["tckimlik"], 0, 1);

          $kuralBir = (($tekhanelertoplamı->tekhanelertoplamı * 7) - $cifthanelerintoplamı->cifthanelerintoplamı) % 10 == substr($satir["tckimlik"], 9, 1);
          $kuralIki = $hanelerintoplam->hanelerintoplamı % 10 == substr($satir["tckimlik"], 10, 1);
